...
/**
 * The main plugin class to be used to interface withthe Daisy repository. It
 * will allow you to search the repository and retrieve a list of documents
 * avaiable.
 */
...
public class RepositoryInterface {
    ...
    /**
     * This searches the repository for all documents and returns a list of them
     * as DOM document
     * 
     * @param document
     */
    public static Document SearchRepository(Document document) throws Exception {
        ...
        String repositoryLocation = org.apache.forrest.repository.daisy.DaisyPlugin
                .getDefault().getPluginPreferences().getString(
                        DaisyPreferences.REPOSITORY_URL);
        String repositoryPort = org.apache.forrest.repository.daisy.DaisyPlugin
                .getDefault().getPluginPreferences().getString(
                        DaisyPreferences.REPOSITORY_PORT);
        String repositoryUsername = org.apache.forrest.repository.daisy.DaisyPlugin
                .getDefault().getPluginPreferences().getString(
                        DaisyPreferences.REPOSITORY_USERNAME);
        String repositoryPassword = org.apache.forrest.repository.daisy.DaisyPlugin
                .getDefault().getPluginPreferences().getString(
                        DaisyPreferences.REPOSITORY_PASSWORD);
        RepositoryManager repositoryManager = new RemoteRepositoryManager(
                "http://" + repositoryLocation + ":" + repositoryPort,
                new Credentials(repositoryUsername, repositoryPassword));
        Repository repository = repositoryManager
                .getRepository(new Credentials(repositoryUsername,
                        repositoryPassword));
        QueryManager queryManager = repository.getQueryManager();
        Object searchresults = queryManager.performQuery(
                "select id, name where true", Locale.getDefault());
        Row[] rows = ((SearchResultDocument) searchresults).getSearchResult()
                .getRows().getRowArray();
        Vector documentList = new Vector();
        for (int i = 0; i < rows.length; i++) {
            String id = rows[i].getValueArray(0);
            String documentName = rows[i].getValueArray(1);
            String name = "http://" + repositoryUsername + ":"
                    + repositoryPassword + "@" + repositoryLocation + ":"
                    + repositoryPort;
